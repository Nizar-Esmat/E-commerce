...
function NewPassword(props) {
    ...
    function handleNewPassWord(e) {
        setLoading(true)
        axios.put("https://ecommerce.routemisr.com/api/v1/auth/resetPassword" ,e)
            .then(({data}) => {
                if(data.token){
                    navigate('/login')
                }
    ...
    let validationSchema = Yup.object({
        ...
        newPassword: Yup.string().matches(/^[A-Z][a-z0-9]{5,10}$/).required(),
    })
    ...
    return (
        <div>
            <h1 className=" text-center my-4 text-green-500">New Password </h1>
            ...
            <form className="max-w-md mx-auto" onSubmit={formik.handleSubmit}>
                ...
                <div className="relative z-0 w-full mb-5 group">
                    <input onBlur={formik.handleBlur} onChange={formik.handleChange} value={formik.values.newPassword}
                           id="newPassword"
                           className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none    dark:border-gray-600 dark:focus:border-green-500 focus:outline-none focus:ring-0 focus:border-green-600 peer"
                           placeholder=" "
                           type="password"/>
                ...
                <button type="submit"
                    ...
                    {loading ? <i className="fa-solid fa-cog fa-spin"></i> : <p>New Password</p>}
                </button>
